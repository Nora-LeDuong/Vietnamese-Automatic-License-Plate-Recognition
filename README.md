...
<<<<<<< HEAD
### 2. Khởi chạy 1 chạm tự động (Khuyên dùng trên Windows)
Dự án đã được tích hợp sẵn kịch bản tự động hóa 100%. Khi người khác **Clone project về**, chỉ cần mở 2 cửa sổ Terminal (hoặc nhấp đúp chuột) chạy song song:

1. **Chạy AI Engine**:
   ```powershell
   .\start-ai-engine.bat
   ```
   *(Tự động tạo môi trường ảo `.venv`, cài đặt `requirements.txt`, chuẩn bị thư mục và khởi chạy FastAPI tại `http://127.0.0.1:8000`)*

2. **Chạy Web Frontend (Laravel)**:
   ```powershell
   .\start-laravel.bat
   ```
   *(Tự động tạo `.env`, chạy `composer install`, tạo `APP_KEY`, khởi tạo `database.sqlite`, chạy `migrate`, liên kết `storage:link`, `npm run build` và khởi chạy server tại `http://127.0.0.1:8001`)*

3. Mở trình duyệt web truy cập: 👉 **`http://127.0.0.1:8001`**

---

### 3. Cài đặt Thủ công (Nếu không dùng file .bat)
Nếu sử dụng Linux/macOS hoặc muốn thiết lập từng bước bằng tay:
* **Backend AI**: `cd ai-engine` -> `python -m venv .venv` -> `source .venv/bin/activate` -> `pip install -r requirements.txt` -> `python src/server.py`
* **Frontend Web**: `cd laravel-app` -> `composer install` -> `copy .env.example .env` -> `php artisan key:generate` -> `type nul > database/database.sqlite` -> `php artisan migrate --force` -> `php artisan storage:link` -> `npm install && npm run build` -> `php -S 127.0.0.1:8001 -t public`
=======
